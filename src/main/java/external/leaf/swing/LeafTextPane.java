 ...
 * @since 2010年5月22日
 */
public class LeafTextPane extends javax.swing.JTextPane {
	/**
	...
	private void initialize() {
		...
		addInputMethodListener(new InputMethodListener() {
			...
			public void inputMethodTextChanged(InputMethodEvent e) {
				...
				try {
					...
					leaf.swing.LeafTextPane.this.isOnIME = (count < length);
				} catch (NullPointerException ex) {
					leaf.swing.LeafTextPane.this.isOnIME = false;
				}
			}
		...
		addPropertyChangeListener("font", new PropertyChangeListener() {
			...
			public void propertyChange(PropertyChangeEvent e) {
				leaf.swing.LeafTextPane.this.met = getFontMetrics(getFont());
			}
		});
	...
	private static final class MyCaret extends DefaultCaret implements PropertyChangeListener {
		...
		private leaf.swing.LeafTextPane pane = null;
		private FontMetrics met;
		...
		public MyCaret(final JTextComponent comp) {
			...
			if (comp instanceof leaf.swing.LeafTextPane) pane = (leaf.swing.LeafTextPane) comp;
		...
		/**
		 * {@link leaf.swing.LeafTextPane}のキャレットを描画します。
		 *
		 * @param g    キャレットを描画するグラフィックス
		 * @param rect キャレットを描画する位置
		 */
		private void paintJTextPaneCaret(Graphics g, Rectangle rect) {
			if (pane.getCaretMode() == leaf.swing.LeafTextPane.CARET_REPLACE_MODE) try {
