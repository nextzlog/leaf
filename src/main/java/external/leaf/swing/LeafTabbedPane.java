 ...
 * @since 2010年3月12日
 */
public class LeafTabbedPane extends javax.swing.JTabbedPane {
	...
	private final int getTargetIndex(Point point) {
		Point ptab = SwingUtilities.convertPoint(
				glasspane, point, leaf.swing.LeafTabbedPane.this);
	...
	private final class TabTitleLabel extends JPanel {
		...
		public void close() {
			...
			if (index >= 0) leaf.swing.LeafTabbedPane.this.remove(index);
		}
		...
		private void initVerticalLine(int i) {
			if (i < 0 || index == i || i - index == 1) {
				...
				Rectangle r = SwingUtilities.convertRectangle(
						leaf.swing.LeafTabbedPane.this, getBoundsAt(0), glasspane);
				cursorRect.setLocation(r.x - LINE_WIDTH / 2, r.y);
				cursorRect.setSize(LINE_WIDTH, r.height);
			} else {
				Rectangle r = SwingUtilities.convertRectangle(
						leaf.swing.LeafTabbedPane.this, getBoundsAt(i - 1), glasspane);
	...
	private final class TabTransferable implements Transferable {
		final String NAME = leaf.swing.LeafTabbedPane.class.getName();
		...
		public Object getTransferData(DataFlavor flavor) {
			return leaf.swing.LeafTabbedPane.this.getTabComponentAt(index);
		}
