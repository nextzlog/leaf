...
final class NameTable{
	...
	*/
	public NameTable(LexicalAnalyzer analyzer){
	...
	*/
	public final class Address{
		...
		*@param name 変数名
		*/
		Address(int nest, int index, boolean isParam, String name){
			this.nest  = nest;
			this.index = index;
	...
	private final class FunctionScope{
		...
		*/
		public Address addLocal(String name){
			return new Address(0, block.add(name), false, name);
		}
		/**
		*スコープ内に引数を追加します。
		*@param name 引数名
		...
		*/
		public Address addParam(String name){
			params.add(name);
			return new Address(0, params.size()-1, true, name);
		}
		/**
		*アクセス可能な変数を検索します。
		*@param name 変数名
		...
		*/
		...
		private Address searchLocal(String name, int nest){
			...
			if(index >= 0)
				return new Address(nest, index, false, name);
			//エンクロージャ内検索
		...
		*/
		public Address searchParam(String name){
			...
			if(index >= 0){
				return new Address(0, index, true, name);
			}
