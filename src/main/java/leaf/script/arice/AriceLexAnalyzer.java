...
final class AriceLexAnalyzer extends AriceCodesAndTokens{
	...
	private final Keyword [] keywords  = Keyword.values();
	...
	*/
	private boolean isOperator(Token token, char ch){
		...
		for(Operator op : operators){
			if(op.toString().indexOf(target)>=0) return true;
		}
		return false;
	}
	/**
	...
	*/
	private Enum getOperator(Token token){
		for(Operator op : operators){
			if(token.equals(op.toString())) return op;
		}
		return null;
	}
	/**
	...
	*/
	private Token readDouble(Token token) throws ScriptException{
		for(; true; index++){
			char ch = charAt(index);
			if(ch == 'd' || ch == 'D'){index++; break;}
			else if(isDigit(ch)) token.append(ch);
			else if(ch != '_') break;
		}
		return token.setType(TokenType.DOUBLE);
	}
