...
/**
*このクラスは、Leafのネットワーク通信機能のメンバーを実装します。<br>
...
public class LeafNetMember implements Runnable,LeafNetMessageListener{
	...
	*/
	public void returnMembers(){
		LeafNetRoom room = server.getRoom(roomname);
		if(room!=null){
			sendMessage("members " + LeafArrayManager.getStringFromList(" ",room.getClientList()));
		}
	}
	/**
	...
	*/
	public void returnRooms(){
		sendMessage("rooms " + LeafArrayManager.getStringFromList(" ",server.getRoomList()));
	}
	/**
